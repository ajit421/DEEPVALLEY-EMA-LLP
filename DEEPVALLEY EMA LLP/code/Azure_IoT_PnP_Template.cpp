...
#define TELEMETRY_PROP_NAME_MOSFETTEMPERATURE "MOStemperature"
#define TELEMETRY_PROP_NAME_PackTemperature "Packtemperature"
#define TELEMETRY_PROP_NAME_PackVolt "packvolt"
#define TELEMETRY_PROP_NAME_CURRENT "current"
#define TELEMETRY_PROP_NAME_CHARGEPERCENTAGE "chargepercentage"
#define TELEMETRY_PROP_NAME_PROTECTIONSTATUS "protectionstatus"
#define TELEMETRY_PROP_NAME_CYCLE "cycle"
...
/* --- Internal Functions --- */
static float get_MOStemperature() { return JbdBms::getTemp1(); } 

static float get_Packtemperature() { return JbdBms::getTemp2(); }

static float get_packvoltage() { return voltage; }

static float get_current() { return JbdBms::getCurrent(); }

static float get_chargepercentage() { return JbdBms::getChargePercentage(); }

static float get_protectionstatus() { return JbdBms::getProtectionState(); }

static float get_cycle() { return JbdBms::getCycle(); }
    ...
    size_t* payload_buffer_length)
{
  ...
  az_span json_span;
  float MOStemperature, Packtemperature, packvolt, current, chargepercentage, protectionstatus, cycle;

  // Acquiring the BMS data.
  MOStemperature = get_MOStemperature();
  Packtemperature = get_Packtemperature();
  packvolt = get_packvoltage();
  current = get_current();
  chargepercentage = get_chargepercentage();
  protectionstatus = get_protectionstatus();
  cycle = get_cycle();
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_MOSFETTEMPERATURE));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding MOSFET temperature property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, MOStemperature, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding MOSFET temperature property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_PackTemperature));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Pack Temperature property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, Packtemperature, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding Pack Temperature property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_PackVolt));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding pack voltage property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, packvolt, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding pack voltage property value to telemetry payload.");
  ...
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding current property value to telemetry payload.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_CHARGEPERCENTAGE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding charge percentage property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, chargepercentage, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding charge percentage property value to telemetry payload.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_CYCLE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cycle property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, cycle, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding cycle property value to telemetry payload.");

  rc = az_json_writer_append_property_name(
      &jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_PROTECTIONSTATUS));
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding protection status property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, protectionstatus);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding protection status property value to telemetry payload.");

  packCellInfoStruct packCellInfo = JbdBms::getPackCellInfo();
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("numOfCells"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding numOfCells property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, packCellInfo.NumOfCells);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding numOfCells property value to telemetry payload.");
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("cellLow"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellLow property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, packCellInfo.CellLow);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellLow property value to telemetry payload.");
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("cellHigh"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellHigh property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, packCellInfo.CellHigh);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellHigh property value to telemetry payload.");
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("cellDiff"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellDiff property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, packCellInfo.CellDiff);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellDiff property value to telemetry payload.");
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("cellAvg"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellAvg property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, packCellInfo.CellAvg);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding cellAvg property value to telemetry payload.");
